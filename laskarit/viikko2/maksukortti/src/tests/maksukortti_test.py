...
class TestMaksukortti(unittest.TestCase):
    ...
    def test_lataa_negatiivinen_maara_ei_muuta_saldoa(self):
        kortti = Maksukortti(1000)
        kortti.lataa_rahaa(-100)

        self.assertEqual(str(kortti), "Kortilla on rahaa 10.00 euroa")

    def test_syo_edullisesti_kun_rahaa_vain_edulliseen_lounaaseen(self):
        kortti = Maksukortti(250)
        kortti.syo_edullisesti()

        self.assertEqual(str(kortti), "Kortilla on rahaa 0.00 euroa")

    def test_syo_maukkaasti_kun_rahaa_vain_maukkaaseen_lounaaseen(self):
        kortti = Maksukortti(400)
        kortti.syo_maukkaasti()
