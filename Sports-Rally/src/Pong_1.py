...
# Stamina handling
def handle_stamina(keys, stamina_blocks, stamina_recharge_timer, stamina_consume_timer, pygame_K_LSHIFT):
    boost_active = False

    if keys[pygame_K_LSHIFT] and stamina_blocks > 0:
        boost_active = True
        ...
        if stamina_consume_timer == 0:
            stamina_blocks = max(0, stamina_blocks - 2)

            if stamina_blocks == 0:
                boost_active = False
            else:
                pass  # We can't call pygame.time.set_timer here, but we can handle this in the main game loop
...
def move_paddles():
    ...
    if ball_dx > 0:  # Opponent paddle moves only when the ball is moving towards it
        target_y = calculate_opponent_target_y(ball.y, opponent_paddle.height, ball.height)
        opponent_paddle.y = adjust_opponent_paddle_y(opponent_paddle.y, target_y, opponent_speed_normal)
